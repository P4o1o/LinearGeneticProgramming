...
#pragma omp critical
    {
        if(rand_engine.index >= STATE_SIZE){
                ...
                for(uint64_t i = 0; i < STATE_SIZE/16; i++){
                    ...
                    __m512i x_next = _mm512_loadu_si512(rand_engine.state + ((idx + 1) % (STATE_SIZE)));
                    ...
                    __m512i lastpiece =  _mm512_loadu_si512(rand_engine.state + ((idx + M) % (STATE_SIZE)));
                ...
                for (uint64_t i = 0; i < STATE_SIZE/8; i++) {
                    ...
                    __m256i x_next = _mm256_loadu_si256((const __m256i*)(rand_engine.state + ((idx + 1) % (STATE_SIZE))));
                    ...
                    __m256i lastpiece = _mm256_loadu_si256((const __m256i*)(rand_engine.state + ((idx + M) % (STATE_SIZE))));
                ...
                for (uint64_t i = 0; i < STATE_SIZE/4; i++) {
                    ...
                    __m128i x_next = _mm_loadu_si128((__m128i*)(rand_engine.state + ((idx + 1) % (STATE_SIZE))));
                    ...
                    __m128i lastpiece = _mm_loadu_si128((__m128i*)(rand_engine.state + ((idx + M) % (STATE_SIZE))));
