...
void elitism_##TYPE(struct Population * initial, const union SelectionParams* params){ \
    ...
    for(uint64_t i = params->size; i < initial->size; i++){ \
        aligned_free(initial->individual[i].prog.content); \
    } \
    ...
    for (uint64_t i = 0; i < tournaments; i++) { \
        ...
        for (uint64_t j = 1; j < params->size; j++) { \
            if (cmp_tournament_##TYPE(initial->individual[i * params->size + winner].fitness, initial->individual[i * params->size + j].fitness)) { \
                aligned_free(initial->individual[i * params->size + winner].prog.content); \
                winner = j; \
            }else{ \
                aligned_free(initial->individual[i * params->size + j].prog.content); \
            } \
        } \
    ...
    if (small_tourn) { \
        ...
        for (uint64_t i = 2; i <= small_tourn; i++) { \
            if (cmp_tournament_##TYPE(initial->individual[initial->size - winner].fitness, initial->individual[initial->size - i].fitness)) { \
                aligned_free(initial->individual[initial->size - winner].prog.content); \
                winner = i; \
            }else{ \
                aligned_free(initial->individual[initial->size - i].prog.content); \
            } \
        } \
    ...
    for (uint64_t i = 0; i < tournaments; i++) { \
        ...
        for (uint64_t j = 1; j < params->fs_params.select_factor.size; j++) { \
            if (cmp_tournament_##TYPE(fs[i * params->fs_params.select_factor.size + winner], fs[i * params->fs_params.select_factor.size + j])) { \
                aligned_free(initial->individual[i * params->fs_params.select_factor.size + winner].prog.content); \
                winner = j; \
            }else{ \
                aligned_free(initial->individual[i * params->fs_params.select_factor.size + j].prog.content); \
            } \
        } \
